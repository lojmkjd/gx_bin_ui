...
def random_test():
    global i
    global test_strings1, test_strings3
    # if test_strings1 == "E,-1,0":
    #     test_strings1 = box[i]
    #     i += 1
    # else:
    #     test_strings1 = "E,-1,0"
...
if __name__ == "__main__":
    ...
    # timer_update_full = threading.Thread(target=update_fulled_value(), name="满载检测")

    app = QApplication(sys.argv)
    w = MainWindow()
    w.show()
    # 设定刷新事件和刷新时间
    # timer_update_full = QTimer(w, timeout=update_fulled_value)
    # timer_update_full.start()

    timer_update_full=threading.Thread(target=update_fulled_value,name="满载检测",daemon=True)
    timer_update_full.start()

    # timer_add_msg = QTimer(w, timeout=add_msg)
    # timer_add_msg.start(20)
