...
class DWaterProgress(QProgressBar):
    ...
    def onTimerOut(self):
        ...
        for pop in self.pops:
            ...
            if pop.yOffset > self.value():
                pop.yOffset = 0
            pop.xOffset = (
                math.sin((pop.yOffset / 100) * 2 * 3.14) * 18 * pop.xSpeed + 50
            )
        self.update()
    ...
    def paint(self, painter):
        ...
        yOffset = (
            rect.toRect().topLeft().y() + (100 - self.value() - 10) * sz.height() / 100
        )
        ...
        # startColor = QColor('#000000')
        startColor.setAlphaF(1)
        endColor = QColor("#50FFF7")
        # endColor = QColor('#000000')
        ...
        waterPainter.drawEllipse(
            waterImage.rect().center(), sz.width() / 2 + 1, sz.height() / 2 + 1
        )
        ...
        waterPainter.drawImage(
            int(self.backXOffset) - self.waterBackImage.width(),
            yOffset,
            self.waterBackImage,
        )
        waterPainter.drawImage(int(self.frontXOffset), yOffset, self.waterFrontImage)
        waterPainter.drawImage(
            int(self.frontXOffset) - self.waterFrontImage.width(),
            yOffset,
        ...
        if self.isTextVisible():
            ...
            if progressText == "100":
                ...
                rectPerent = QRect(
                    QPoint(
                        rectValue.right(), rectValue.bottom() - rect.height() * 20 / 100
                    ),
                    QPoint(
                        rectValue.right() + rect.width() * 20 / 100, rectValue.bottom()
                    ),
                )
